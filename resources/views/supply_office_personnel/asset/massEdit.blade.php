...
@section('content')
            ...
            <tbody>
                @foreach ($assets as $i => $asset)
                    <tr>
                        ...
                        @endif
                        <td>
                            <input type="hidden" name="serviceable[]" value="{{old('.'.$i) ?? $asset->serviceable}}">
                            <input type="checkbox" class="service" {{$asset->serviceable ? 'checked' : ''}}>
                        </td>
                        <td>
    ...
    <script>
        ...
        $('#assetsTable').on('change', 'input.service', function() {
            $(this).parent().children()[0].value = $(this).is(':checked');
        });

...
@section('content')
    ...
    @foreach ($requested_jobs as $job)
        ...
        <h5>
            {{$job->asset_name}}
            <br>
            {{$job->department_name}} <a href="{{route('employee.job.edit', ['job' => $job->job_id])}}">Edit</a> 
    ...
    @foreach ($job_to_verify as $job)
        ...
        <a href="{{route('employee.job.verify', ['job' => $job->job_id])}}">Verify</a>
    @endforeach
        ...
        $('#delete').click(function(){
            $('#jobDeleteModal').on('show.bs.modal', function(e) {
                var job_id = $(e.relatedTarget).data('job_id'),
                    baseUrl =  '{{url('')}}',
                    url = baseUrl + "/employee/job/" + job_id;
